...
esp_err_t save_run_time(void)
{
    ...
    err = nvs_get_u32(my_handle, "dummyU32Key", NULL);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;
    ...
    if (required_size > 0) {
        err = nvs_get_u32(my_handle, "dummyU32Key", run_time);
        if (err != ESP_OK) {
            free(run_time);
            return err;
        }
    ...
    run_time[required_size / sizeof(uint32_t) - 1] = xTaskGetTickCount() * portTICK_PERIOD_MS; //makes this number appear at the end, regardleses of size of run_time
...
esp_err_t print_what_saved(void)
{
    ...
    // Read run time blob
    size_t required_size = sizeof(uint32_t);  // value will default to 0, if not set yet in NVS
    // obtain required memory space to store blob being read from NVS
    err = nvs_get_u32(my_handle, "onrer", NULL);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;
    printf("Run time:\n");
    if (required_size == 0) {
        printf("Nothing saved yet!\n");
    } else {
        uint32_t* run_time = malloc(sizeof(uint32_t));
        err = nvs_get_u32(my_handle, "onrer", run_time);
        if (err != ESP_OK) {
            free(run_time);
            return err;
        }
