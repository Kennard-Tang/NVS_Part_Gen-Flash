...
esp_err_t save_restart_counter(void)
{
    ...
    //get required size 
    err = nvs_get_str(my_handle, "serialNo", NULL, &str_size);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;

    // Read
    err = nvs_get_i32(my_handle, "restart_conter", &restart_counter);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;
    err = nvs_get_str(my_handle, "serialNo", dummystr, &str_size);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;
...
esp_err_t print_what_saved(void)
{
    ...
    // Read serial_no
    char dummystr[] = "test"; //if not set yet in NVS
    ...
    {
        uint32_t* run_time = malloc(str_size);
        err = nvs_get_str(my_handle, "serialNo", dummystr, &str_size);
        if (err != ESP_OK) {
            free(run_time);
            return err;
    ...
    int32_t restart_counter = 0; // value will default to 0, if not set yet in NVS
    err = nvs_get_i32(my_handle, "r", &restart_counter);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;
